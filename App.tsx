...
import { ProductInfo, ShoppingListItem } from './types';
import { ShoppingBag, Search, Scan, History, Heart, Loader2, Sparkles, X } from 'lucide-react';
        ...
        {!scannedProduct ? (
          <div className="flex flex-col items-center gap-12">
            <div className="text-center space-y-4 px-4">
              <h2 className="text-4xl md:text-5xl font-black text-gray-900">اتخذ قراراً ذكياً بمجرد مسحة</h2>
          ...
          <div className="relative bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b flex items-center justify-between">
              <h3 className="text-xl font-bold flex items-center gap-2">
                ...
                <span>قائمة التسوق الخاصة بك</span>
              </h3>
      ...
      <nav className="fixed bottom-6 left-4 right-4 z-40">
        <div className="max-w-md mx-auto bg-gray-900/90 backdrop-blur-xl rounded-full p-2 shadow-2xl flex items-center justify-around border border-white/10">
          <button 
            ...
            className="flex-1 flex flex-col items-center gap-1 py-2 text-indigo-400"
          >
            <Scan className="w-6 h-6" />
            <span className="text-[10px] font-bold">المسح</span>
          </button>
          <button className="flex-1 flex flex-col items-center gap-1 py-2 text-gray-400 hover:text-white transition">
            <History className="w-6 h-6" />
            <span className="text-[10px] font-bold">السجل</span>
          </button>
          <button 
            onClick={() => setShowList(true)}
            className="flex-1 flex flex-col items-center gap-1 py-2 text-gray-400 hover:text-white transition"
          >
